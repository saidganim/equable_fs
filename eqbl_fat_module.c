...
void destroy_efat_inode(struct inode *inode){
								kmem_cache_free(efat_inode_cachep, EFAT_INODE(inode));
};
...
static int eqbl_fat_fill_sb( struct super_block* sb, void* data, int silent){
								...
								if (unlikely(__efat_sb->magic != EQBL_FAT_MAGIC_NUMBER)) {
																EFS_LOG("EQBL_FAT_FS_ERR: The magic number on device doesn't match magic number of FS...\n");
																...
																efat_write_cluster(sb, (char *)__efat_sb, EQBL_FAT_SUPER_BLOCK_OFFSET);
																// ret = -EACCES;
																//goto release;
								}
...
static struct dentry *eqbl_fat_mount(struct file_system_type *fs_type_struct,
...
static struct file_system_type eqbl_fat_struct = {
								...
								.name = EQBL_FAT_NAME,
								// mounting filesystem
								.mount = eqbl_fat_mount,
								// kill_block_super is provided by the kernel
								.kill_sb = kill_block_super,
								// eqbl_fat needs disk for work
...
static int __init eqbl_fat_registrating(void)
{
								...
								efat_inode_cachep = KMEM_CACHE(
																efat_inode,
																(SLAB_RECLAIM_ACCOUNT | SLAB_MEM_SPREAD));

								if (unlikely(!efat_inode_cachep)) {
																EFS_LOG("EFAT: Couldn't Allocate memory "
																								"for slab cache\n");
																return -ENOMEM;
								}
								// registrating the filesystem in the kernel
								return register_filesystem(&eqbl_fat_struct);
}
